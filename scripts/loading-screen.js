...
class LoadingScreenManager {
  ...
  static async getImagesForScene(scene) {
    ...
    const sceneFolders = game.settings.get(this.ID, this.SETTINGS.SCENE_FOLDERS);
    ...
    try {
      ...
      return scene?.background?.src ? [scene.background.src] : ['icons/svg/clockwork.svg'];
    }
  }
  ...
  static async browseFolder(folderPath) {
    ...
    try {
      ...
      const FilePicker = foundry.applications?.apps?.FilePicker?.implementation || window.FilePicker;
      ...
      console.warn('Loading Screen | Konnte Ordner nicht durchsuchen:', folderPath, error);
      return [];
    }
  }
  ...
  static async showLoadingScreen(scene) {
    ...
    const showProgress = game.settings.get(this.ID, this.SETTINGS.SHOW_PROGRESS);
    ...
    const sceneName = scene?.name || game.i18n.localize('LOADING_SCREEN.Loading');

    // Bereite Tipps vor
    let currentTip = '';
    if (showTips) {
      this._currentTips = this.getTips();
      this._currentTipIndex = Math.floor(Math.random() * this._currentTips.length);
  ...
  static async renderTemplate(templateName, data) {
    const templatePath = `modules/nwc-loadingscreen/templates/loading-screens/${templateName}.html`;
    
    try {
      const template = await getTemplate(templatePath);
      return template(data);
    } catch (error) {
      console.error(`Loading Screen | Template '${templateName}' not found, falling back to standard`, error);
      // Fallback zu Standard-Template
      const standardTemplate = await getTemplate('modules/loading-screen/templates/loading-screens/standard.html');
  ...
  static startTipRotation() {
    const rotationInterval = game.settings.get(this.ID, this.SETTINGS.TIP_ROTATION);
  ...
  static rotateTip() {
    ...
    setTimeout(() => {
      ...
      this._currentTipIndex = (this._currentTipIndex + 1) % this._currentTips.length;
